...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      charityOrgId: null,
      zoneRules: [
        (v) => !!v || i18n.t('zones.zone') + i18n.t('required.null'),
        (v) =>
          (v && v.length <= 250) ||
          i18n.t('zones.zone') + i18n.t('required.length-250'),
      ],
    }),
    computed: {
      status: function () {
      ...
      modeI18Title: function () {
        switch (this.option) {
          case 1:
            return i18n.t('zones.zone-create');
          case 2:
            return i18n.t('zones.zone-show');
          case 3:
            return i18n.t('zones.zone-edit');
          default:
            return 0;
        }
      },
    ...
    async mounted () {
      ...
      if (this.option === undefined) {
        this.$router.push({ name: 'Inquiiess' });
      }
    },
    ...
    methods: {
      initialize () {
        ...
        if (this.option === 3 || this.option === 2) {
          ...
          this.zone = Object.assign(zone, {
            ...
            nombreZona: paramData.nombreZona,
          });
          ...
          if (this.details.length > 0) {
            ...
            if ((cont < 100 || cont > 100) && sum) {
              ...
              if (this.option === 1) {
                ...
                let responseData = await putInquity(data, this.zone.id);
                if (responseData.ok === 1) {
                  this.$toasted.show(responseData.message.text, {
                    theme: 'outline',
                    position: 'top-right',
      ...
      descriptionRules (value) {
        if (!value || value.length <= 200) {
          ...
          return i18n.t('zones.description') + i18n.t('required.length-200');
        }
      },
    },
  };
