              ...
              >
                <v-container class="max-full-width">
                  <v-row>
                    ...
                    >
                      <v-text-field
                        v-model="incident.idUsuario"
                        class="purple-input"
                        :label="$t('incidents.user') + ' (*)'"
                        :readonly="!isSaveMode"
                        :rules="userRules"
                      />
                    </v-col>
                    ...
                    >
                      <v-text-field
                        v-model="incident.incidentFecha"
                        class="purple-input"
                  ...
                  <v-row
                    v-if="incident.lstEvidencias.length > 0"
                    class="mt-3"
                  >
                    <p>Evidencias</p>
                  </v-row>
                  ...
                  >
                    <v-col
                      v-for="(item, index) in incident.lstEvidencias"
                      :key="index"
                      cols="12"
                      md="4"
                      class="text-center"
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      option: null,
      incidentRules: [
        (v) => !!v || i18n.t('incidents.incident') + i18n.t('required.null'),
        (v) =>
          (v && v.length <= 250) ||
          i18n.t('incidents.incident') + i18n.t('required.length-250'),
      ],
      userRules: [
    ...
    computed: {
      modeI18Title: function () {
        switch (this.option) {
          case 1:
            return i18n.t('incidents.incident-create');
          case 2:
            return i18n.t('incidents.incident-show');
          case 3:
            return i18n.t('incidents.incident-edit');
          default:
            return 0;
        }
      },
    ...
    methods: {
      initialize () {
        ...
        if (this.option === 3 || this.option === 2) {
          ...
          this.incident = Object.assign(incident, {
            idIncident: paramData.idIncident,
            idDespacho: paramData.idDespacho,
            incidentFecha: moment(paramData.incidentFecha).format('DD/MM/YYYY HH:mm'),
            idUsuario: paramData.idUsuario,
            descripcion: paramData.descripcion,
            lstEvidencias: paramData.lstEvidencias,
          });
        }
      ...
      onSave: async function () {
        if (this.$refs.form.validate()) {
          const requestData = Object.assign(incident, {
            idIncident: this.incident.idIncident,
            idDespacho: this.incident.idDespacho,
            incidentFecha: this.incident.incidentFecha,
            idUsuario: this.incident.idUsuario,
            descripcion: this.incident.descripcion,
            lstEvidencias: this.incident.lstEvidencias,
          });
      ...
      descriptionRules (value) {
        if (!value || value.length <= 250) {
          ...
          return i18n.t('incidents.description') + i18n.t('required.length-250');
        }
      },
    },
  };
